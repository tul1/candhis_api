...
func TestAddWaveData_Success(t *testing.T) {
	...
	err := waveDataStore.AddWaveData(context.Background(), waveData, "test-index")
	assert.NoError(t, err)
}

func TestAddWaveData_Error(t *testing.T) {
	waveDataStore := setupMockWaveData(func(req *http.Request) (*http.Response, error) {
...
func TestAddWaveData_EmptyIndexName(t *testing.T) {
	waveDataStore := setupMockWaveData(func(req *http.Request) (*http.Response, error) {
		return MockResponse(201, `{"result": "created"}`), nil
	})
	...
	err := waveDataStore.AddWaveData(context.Background(), waveData, "")
