...
func TestGatherWaveDataFromWebTable_Success(t *testing.T) {
	mockHandler := func(req *http.Request) *http.Response {
		return MockHTTPResponse(200, mockHTMLResponse)
	}
	scrapper := setupMockCandhisScrapper(t, mockHandler)

	waveData, err := scrapper.GatherWaveDataFromWebTable("test_sessid", "http://fake.url")
...
func TestGatherWaveDataFromWebTable_EmptyResponse(t *testing.T) {
	mockHandler := func(req *http.Request) *http.Response {
...
func setupMockCandhisScrapper(t *testing.T, mockHandler func(req *http.Request) *http.Response) repo.CandhisWebScrapper {
	...
	return client.NewCandhisWebScrapper(mockClient)
}
