...
func TestWaveData_AddWaveData_Success(t *testing.T) {
	...
	err := waveDataStore.AddWaveData(ctx, waveData1, "wave_data_test")
	require.NoError(t, err)
	err = waveDataStore.AddWaveData(ctx, waveData2, "wave_data_test")
	require.NoError(t, err)

	retrievedWaveDataList := persistor.WaveData().ListWaveData(context.Background(), "wave_data_test")
...
func setupWaveDataTest(t *testing.T) (*persistencetest.ESPersistor, repository.WaveData) {
	...
	waveDataStore := persistence.NewWaveData(es)
	...
	return persistor, waveDataStore
}
